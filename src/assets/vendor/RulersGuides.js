...
var RulersGuides = function (evt, dragdrop,containerWidth) {
    'use strict';
    containerWidth = containerWidth || 1080;//默认宽度1080
    ...
    //Ruler     = function (type, size) {
        Ruler       = function (type, size ,leftWidth) {
            ...
            /*start*/
            if(type == "h") {
                yxj = Math.floor((leftWidth[0]-containerWidth)/2)+15;//先以1080计算为基础
...
exports.rulerInit = function (_this,containerWidth) {
    var evt         = new rulerEvent(),
        dragdrop    = new Dragdrop(evt),
        rg          = new RulersGuides(evt, dragdrop,containerWidth);
    alert(_this.websiteInfo.pageWidth)
    if(_this.websiteInfo.pageWidth=='1000px'){
        $(".ruler.h").css("left",'-50px')
    }else if(_this.websiteInfo.pageWidth=='1200px'){
        $(".ruler.h").css("left",'50px')
    }else{
        $(".ruler.h").css("left",'0px')
    }
