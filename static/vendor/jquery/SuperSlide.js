(function( factory ) {
	if ( typeof define === "function" && define.amd ) {
		...
		define([ "../../../src/assets/vendor/jquery/jquery" ], factory );
	} else {
		// Browser globals
		factory( jQuery );
	}
	...
	$.fn.slide = function (options) {
        ...
        return this.each(function () {
            ...
            var wrap = $(opts.wrap, slider);
            slider.on("slideDestroy",function () {
                clearInterval(inter)
            ...
            var doStartFun = function () {
                if ($.isFunction(opts.startFun)) {
                    opts.startFun(inter,index, navObjSize, slider, $(opts.titCell, slider), conBox, tarObj, prevBtn, nextBtn)
                }
            }
            ...
            //前后按钮事件
            if (isMarq) {
                nextBtn.bind('mousedown', nextTrigger);
                prevBtn.bind('mousedown', prevTrigger);
                //前后按钮长按10倍加速
                if (pnLoop) {
                    var st;
                    ...
                    nextBtn.bind('mousedown', marDown);
                    nextBtn.bind('mouseup', marUp);
                    prevBtn.bind('mousedown', marDown);
                    prevBtn.bind('mouseup', marUp);
                }
                //前后按钮mouseover事件
                if (opts.trigger == "") {
                ...
                nextBtn.bind('click', nextTrigger);
                prevBtn.bind('click', prevTrigger);
            }
